# Use singleton class Amz::Preferences::Store.instance to access
#
...
module Amz::Preferences
  class StoreInstance
    ...
    def exist?(key)
      @cache.exist?(key) ||
        should_persist? && Amz::Preference.where(key: key).exists?
    end
    ...
    def get(key)
      ...
      if should_persist?
        ...
        val = if preference = Amz::Preference.find_by(key: key)
                # it does exist
                preference.value
              else
    ...
    def persist(cache_key, value)
      ...
      preference = Amz::Preference.where(key: cache_key).first_or_initialize
    ...
    def destroy(cache_key)
      ...
      preference = Amz::Preference.find_by(key: cache_key)
      preference&.destroy
    end

    def should_persist?
      @persistence && Amz::Preference.table_exists?
